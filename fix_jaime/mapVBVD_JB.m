function twix_obj = mapVBVD_JB(filename,varargin)

%  Reads Siemens raw .dat file from VB/VD MRI raw data.
%  requires twix_map_obj_JB.m & read_twix_hdr_JB.m
%
%
...
%                          twix_map_obj_JB.m now stores the memory position of
...
if and(firstInt < 10000, secondInt <= 64)
    version = 'vd';
    disp('Software version: VD/VE/XA (!?)');
...
for s=1:NScans
    ...
    if arg.bReadHeader
        [twix_obj{s}.hdr, rstraj] = read_twix_hdr_JB(fid);
    ...
    isPrescan = false;
    try
        if ( (twix_obj{s}.hdr.MeasYaps.sPreScanNormalizeFilter.ucOn == 1) && (s==1) )
            isPrescan=true;
        end
    catch
        %donothing
    ...
    twix_obj{s}.image         = twix_map_obj_JB(arg,'image',filename,version,rstraj);
    twix_obj{s}.noise         = twix_map_obj_JB(arg,'noise',filename,version);
    twix_obj{s}.phasecor      = twix_map_obj_JB(arg,'phasecor',filename,version,rstraj);
    twix_obj{s}.phasestab     = twix_map_obj_JB(arg,'phasestab',filename,version,rstraj);
    twix_obj{s}.phasestabRef0 = twix_map_obj_JB(arg,'phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.phasestabRef1 = twix_map_obj_JB(arg,'phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.refscan       = twix_map_obj_JB(arg,'refscan',filename,version,rstraj);
    twix_obj{s}.refscanPC     = twix_map_obj_JB(arg,'refscan_phasecor',filename,version,rstraj);
    twix_obj{s}.refscanPS     = twix_map_obj_JB(arg,'refscan_phasestab',filename,version,rstraj);
    twix_obj{s}.refscanPSRef0 = twix_map_obj_JB(arg,'refscan_phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.refscanPSRef1 = twix_map_obj_JB(arg,'refscan_phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.RTfeedback    = twix_map_obj_JB(arg,'rtfeedback',filename,version,rstraj);
    twix_obj{s}.vop           = twix_map_obj_JB(arg,'vop',filename,version); % tx-array rf pulses
    twix_obj{s}.prescanbody = twix_map_obj_JB(arg,'prescanbody',filename,version,rstraj); %add prescan with body coil
    twix_obj{s}.prescansurface = twix_map_obj_JB(arg,'prescansurface',filename,version,rstraj); %add prescan with surface coil
    ...
    [mdh, mask] = evalMDH( mdh_blob, version, isInplacePATref,isPrescan); % this is quasi-instant (< 1s) :-)    
    clear mdh_blob
    ...
    % prescan
    if isPrescan && arg.bReadPrescan
        if s==1
            clear tmpMdh
            
            isCurrScan = logical( mask.MDH_prescanbody );
            for f = fieldnames( mdh ).'
                tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
            end
            twix_obj{s}.prescanbody.readMDH( tmpMdh, filePos(isCurrScan) );

                    clear tmpMdh
            isCurrScan = logical( mask.MDH_prescansurface );
            for f = fieldnames( mdh ).'
                tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
            end
            twix_obj{s}.prescansurface.readMDH( tmpMdh, filePos(isCurrScan) );
        end
    end
...
function [mdh,mask] = evalMDH( mdh_blob, version, isInplacePATref, isPrescan )
...
if isPrescan
    mask.MDH_prescanbody              = mdh.ushUsedChannels==2;
    
    tempmask=false(Nmeas,1);
    %sssr: does that mean we only use the 2nd body coil?
    cind=find(mdh.ushUsedChannels==2);
    tempmask(cind-1)=true;
    mask.MDH_prescansurface              = tempmask;
end
