classdef twix_map_obj_lee < handle
...
methods
    % Constructor:
    function this = twix_map_obj_lee(arg,dataType,fname,version,rstraj)
...
%         end
    ...
    function this = clean(this)
        ...
        if strcmp(this.dataType,'refscan') || strcmp(this.dataType,'image')
            ...
            %sssr: these dimensions are assumed to loop from within the line loop
            % other dimensions (rep, set, etc) are hopefully outside the line loop
            Nacqinnerdims = this.NPar*this.NAve*this.NEco;
            % THIS BLOCK FIXES THE 16-bits LINE/PAR ENCODING LIMIT
            Noverflow= 1+floor((this.NAcq-(Nacqinnerdims*65536))/65536);
            if(Noverflow>0)
                %loop over each overflow to lift it up by 2^16
                for of=1:(Noverflow)
                    this.Lin((65536*of*Nacqinnerdims)+1:end) = this.Lin((65536*of*Nacqinnerdims)+1:end) + 65536;
                    if(this.NPar>=(65536*of+1))
                        this.Par((65536*of*Nacqinnerdims)+1:end) = this.Par((65536*of*Nacqinnerdims)+1:end) + 65536;
                    end
                    this.NLin = max(this.Lin);
                    this.NPar = max(this.Par);
                end
    ...
    function out = readData(this,mem,cIxToTarg,cIxToRaw,selRange,selRangeSz,outSize)
        ...
        slicedata    = this.slicePos(:, cIxToRaw); % retrieve slice position
